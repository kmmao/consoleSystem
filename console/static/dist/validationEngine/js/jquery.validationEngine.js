	 ...
	 var methods = {
		...
		*/
		_validateField: function(field, options, skipAjaxValidation) {
			...
			//new add by daiwen ��ʽΪignore��Ԫ�ز�����֤
			...
			for (var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				if (typeof errorMsg == 'string') {
					...
					field_errors++;
					if(field.attr("data-errormessage"))
						break;
				}	
			}
			// If the rules required is not added, an empty field is not validated
			if(!required && !nullValidate && field.val().length < 1) options.isError = false;
		...
		*/
		_past: function(form, field, rules, i, options) {
			...
			if (vdate > pdate ) {
				...
				var dateString = "";
				var dateType = methods._dateType(field.val());
				dateString =  methods._dateFormat(pdate,dateType);
				if (rule.alertText2) return rule.alertText + dateString + rule.alertText2;
				return rule.alertText + dateString;
			}
		},
		/**
		...
		*/
		_future: function(form, field, rules, i, options) {
			...
			if (vdate < pdate ) {
				...
				dateString =  methods._dateFormat(pdate,dateType);
				if (rule.alertText2)
					return rule.alertText + dateString  + rule.alertText2;
				return rule.alertText + dateString;
			}
		},
		/**
		...
		*/
		...
		_dateType : function(dateString){//�ж����ڸ�ʽ
			if(/^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])\s+([0-1]?[0-9]|2[0-3]){1}:([0-5]?[0-9]){1}:([0-5]?[0-9])?$/.test(dateString)){
			 	return "yyyy-MM-dd hh:mm:ss";
		 	}
			else if(/^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])\s+([0-1]?[0-9]|2[0-3]){1}:([0-5]?[0-9])?$/.test(dateString)){
